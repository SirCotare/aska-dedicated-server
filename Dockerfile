...
# healthcheck
HEALTHCHECK --interval=5s --start-period=60s --start-interval=15s CMD (grep -q "Connected to chat" /tmp/app.stdout) && ! (grep -Eq "Uploading Crash Report|A crash has been intercepted by the crash handler" /tmp/app.stdout) || exit 1
